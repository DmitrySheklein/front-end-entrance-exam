...
export function Education() {
  return Box({
    ...
    <ul class="education-list">
      ${renderList(
        educationList.map(({ date, name, schoolName, tags }, idx) => {
              ...
              <div class="education-item">
                <header class="education-item__header">
                  <span class="education-item__date">
                    ${renderList(
                      date.map(
                        (el) =>
                          `<time datetime="${el}" contenteditable>${el}</time>`,
                      ),
                      ' - ',
                    )}
                  </span>
                ...
                <div class="education-item__content">
                  ...
                  <ul class="education-item__tags ${isFirstItem ? 'education-item__tags--accent' : ''}">
                    ${renderList(tags.map((tag) => `<li class="education-item__tag" contenteditable>#${tag}</li>`))}                  
                  </ul>
                </div>
